...
public class NotificationOrderChainMapperTests
{
    ...
    [Fact]
    public void MapToNotificationOrderChainRequest_WithEmailRecipientAndSingleReminder_MapsCorrectly()
    {
        // Arrange
        var creatorName = "ttd";
        ...
        var requestExt = new NotificationOrderChainRequestExt
        {
            ...
            IdempotencyId = "DB9B377F-BD2E-4C2B-ACA9-C27E12E2579E",
            Recipient = new NotificationRecipientExt
            {
                RecipientEmail = new RecipientEmailExt
                {
            ...
            [
                new NotificationReminderExt
                {
                    ...
                    ConditionEndpoint = new Uri("https://vg.no/reminder-condition"),
                    Recipient = new NotificationRecipientExt
                    {
                         RecipientEmail = new RecipientEmailExt
                         {
                             ...
                             Settings = new EmailSendingOptionsExt
                             {
                                 ...
                                 SenderEmailAddress = "Reminder sender address",
                                 ContentType = EmailContentTypeExt.Plain
                             }
                         }
                    }
        ...
        Assert.Equal(1, reminder.DelayDays);
        Assert.Equal("12236E1A-C7D9-4334-8CEE-873DAA64467F", reminder.SendersReference);
        Assert.Equal(requestExt.Reminders[0].ConditionEndpoint, reminder.ConditionEndpoint);

        // Verify reminder time calculation
        var expectedReminderTime = baseTime.AddDays(1).ToUniversalTime();
        Assert.Equal(expectedReminderTime, reminder.RequestedSendTime);

        // Verify reminder recipient
        Assert.NotNull(reminder.Recipient.RecipientEmail);
        Assert.Equal("Reminder body", reminder.Recipient.RecipientEmail.Settings.Body);
        Assert.Equal("Reminder subject", reminder.Recipient.RecipientEmail.Settings.Subject);
        Assert.Equal("recipient@example.com", reminder.Recipient.RecipientEmail.EmailAddress);
        Assert.Equal("Reminder sender name", reminder.Recipient.RecipientEmail.Settings.SenderName);
        Assert.Equal(EmailContentType.Plain, reminder.Recipient.RecipientEmail.Settings.ContentType);
        Assert.Equal("Reminder sender address", reminder.Recipient.RecipientEmail.Settings.SenderEmailAddress);

        // Verify reminder has a unique OrderId
        Assert.NotEqual(Guid.Empty, reminder.OrderId);
        Assert.NotEqual(result.OrderId, reminder.OrderId);
    }
