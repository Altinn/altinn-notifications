...
public class NotificationStatusRetryConsumerBaseTests
{
    [Collection("NotificationStatusRetryConsumerBase-Test1")]
    public class ConsumeRetryMessage_WhenDeserializationReturnsNull_Tests
    {
        [Fact]
        public async Task ConsumeRetryMessage_WhenDeserializationReturnsNull_NoProcessingOccurs()
        {
            ...
            try
            {
                ...
                var kafkaSettings = BuildKafkaSettings(emailStatusUpdatedRetryTopicName);
                ...
                // Act
                await KafkaUtil.PublishMessageOnTopic(kafkaSettings.Value.EmailStatusUpdatedRetryTopicName, "null");
        ...
        private static IOptions<KafkaSettings> BuildKafkaSettings(string emailStatusUpdatedRetryTopicName)
        {
            return Options.Create(new KafkaSettings
            {
                Admin = new AdminSettings()
                {
                    TopicList = [emailStatusUpdatedRetryTopicName]
                },
    ...
    [Collection("NotificationStatusRetryConsumerBase-Test2")]
    public class ConsumeRetryMessage_WhenExceededRetryThreshold_Tests
    {
        [Fact]
        public async Task ConsumeRetryMessage_WhenExceededRetryThreshold_InsertDeadDeliveryReport()
        {
            ...
            try
            {
                ...
                // Act
                await emailStatusConsumer.StartAsync(CancellationToken.None);
                await KafkaUtil.PublishMessageOnTopic(kafkaSettings.Value.EmailStatusUpdatedRetryTopicName, updateStatusRetryMessage.Serialize());
                ...
                await IntegrationTestUtil.EventuallyAsync(
                    () =>
                    {
                        try
                        {
                            ...
                            kafkaProducer.Verify(e => e.ProduceAsync(It.Is<string>(e => e == kafkaSettings.Value.EmailStatusUpdatedRetryTopicName), It.IsAny<string>()), Times.Never);
    ...
    [Collection("NotificationStatusRetryConsumerBase-Test3")]
    public class ConsumeRetryMessage_WhenWithinRetryThreshold_Tests
    {
        [Fact]
        public async Task ConsumeRetryMessage_WhenWithinRetryThresholdAndUpdateFails_MessageRepublishedWithIncrementedAttempts()
        {
            ...
            try
            {
                ...
                kafkaProducer
                    .Setup(p => p.ProduceAsync(kafkaSettings.Value.EmailStatusUpdatedRetryTopicName, It.IsAny<string>()))
                ...
                await IntegrationTestUtil.EventuallyAsync(
                    () =>
                    {
                        try
                        {
                            ...
                            kafkaProducer.Verify(p => p.ProduceAsync(kafkaSettings.Value.EmailStatusUpdatedRetryTopicName, It.IsAny<string>()), Times.Once);
