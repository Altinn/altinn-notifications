...
import * as smsNotificationsApi from "../api/notifications/notifications_sms.js";
import * as ordersApi from "../api/notifications/orders.js";
import * as smsOrdersApi from "../api/notifications/orders_sms.js";
import { uuidv4 } from "https://jslib.k6.io/k6-utils/1.4.0/index.js";
const orderRequestJson = JSON.parse(
  open("../data/orders/01-sms-request.json")
);
...
function TC01_PostSmsNotificationOrderRequest(data) {
  ...
  response = smsOrdersApi.postSmsNotificationOrder(
...
function TC04_GetNotificationOrderWithStatus(data, orderId) {
  ...
  success = check(JSON.parse(response.body), {
    "GET notification order with status. Id property is a match": (order) =>
      order.id === orderId,
    "GET notification order with status. NotificationChannel is sms": (
      order
...
function TC05_GetSmsNotificationSummary(data, orderId) {
  ...
  response =   smsNotificationsApi.getSmsNotifications(orderId, data.token);
  success = check(response, {
    "GET sms notifications. Status is 200 OK": (r) =>
      r.status === 200,
  });
  ...
  success = check(JSON.parse(response.body), {
    "GET sms notifications. OrderId property is a match": (notificationSummary) =>
