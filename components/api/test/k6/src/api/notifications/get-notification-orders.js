...
export function getNotificationOrderById(data, selfLink, orderId) {
    ...
    check(response, {
        "GET notification order by id. Status is 200 OK": (r) =>
            r.status === 200,
    });
...
export function getNotificationOrderBySendersReference(data) {
    const response = ordersApi.getBySendersReference(
 ...
 */
export function getNotificationOrderWithStatus(
    data,
    orderId,
    expectedNotificationChannel
) {
    ...
    check(JSON.parse(response.body), {
        "GET notification order with status. Id property is a match": (order) =>
            order.id === orderId,
        [`GET notification order with status. NotificationChannel is ${expectedNotificationChannel.toLowerCase()}`]:
            (order) =>
                order.notificationChannel === expectedNotificationChannel,
