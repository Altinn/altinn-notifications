...
export async function getFromSecretSource(secretName) {
    ...
    try {
        ...
        if (message.includes("no secret sources are configured")) {
            throwConfigurationError("No secret source is configured for the k6 command - specify the file path with the --secret-source flag");
        }
        else if (message.includes("no value")) {
            throwConfigurationError(`Secret ${secretName} does not exist in the secret source`);
        }
        throwConfigurationError(`Unknown error occurred while reading secret '${secretName}': ${message}`);
    }
    if (!secretValue) {
        throwConfigurationError(`Secret ${secretName} is not properly assigned in the secret source`);
    }
    return secretValue;
}