...
public class OrderRequestServiceTests
{
    ...
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUser_ForSms_UsesSmsContactPointLookupAndCreatesOrderChain()
    {
        ...
        const string externalIdentity = "urn:altinn:person:idporten-email:user@altinn.xyz";
        ...
        var expectedNotificationOrder = new NotificationOrder
        {
            ...
            Recipients = [new Recipient([], externalIdentity: externalIdentity)]
        };
        ...
        contactPointServiceMock
            .Setup(e => e.AddSmsContactPoints(It.Is<List<Recipient>>(r => r.Any(rec => rec.ExternalIdentity == externalIdentity)), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
                    if (recipient.ExternalIdentity == externalIdentity)
                    {
        ...
        contactPointServiceMock.Verify(
            cp => cp.AddEmailContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);

        contactPointServiceMock.Verify(
            cp => cp.AddEmailAndSmsContactPointsAsync(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);

        contactPointServiceMock.Verify(
            cp => cp.AddPreferredContactPoints(It.IsAny<NotificationChannel>(), It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUser_ForEmailPreferredChannel_UsesPreferredLookupAndCreatesOrderChain()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        var expectedSavedOrder = new NotificationOrder
        {
            Id = orderId,
            Created = currentTime,
            Creator = new Creator("ttd"),
            ...
            SendingTimePolicy = SendingTimePolicy.Daytime,
            RequestedSendTime = currentTime.AddMinutes(10),
            SendersReference = "self-identified-preferred-ref",
        ...
        contactPointServiceMock
            .Setup(cp => cp.AddPreferredContactPoints(
                ...
                It.IsAny<Creator>(),
                It.IsAny<string?>()))
            .Callback<NotificationChannel, List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((_, recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
                    if (recipient.ExternalIdentity == externalIdentity)
                    {
                        recipient.IsReserved = false;
                        recipient.AddressInfo.Add(new EmailAddressPoint("user@altinn.xyz"));
                    }
                }
            })
            .Returns(Task.CompletedTask);
        ...
        contactPointServiceMock.Verify(
            cp => cp.AddSmsContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), It.IsAny<OrderLifecycleStage>(), It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);

        contactPointServiceMock.Verify(
            cp => cp.AddEmailContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), It.IsAny<OrderLifecycleStage>(), It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);
    ...
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUserMissingContact_ReturnsProblem()
    {
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(cp => cp.AddEmailContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((_, _, _, _, _) =>
            {
        ...
        contactPointServiceMock.Verify(cp => cp.AddEmailContactPoints(It.Is<List<Recipient>>(r => r.Any(rec => rec.ExternalIdentity == externalIdentity)), It.Is<string?>(s => s == null), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()), Times.Once);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUser_ForEmail_UsesEmailContactPointLookupAndCreatesOrderChain()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        const string resourceId = "urn:altinn:resource:test-resource";
        const string externalIdentity = "urn:altinn:person:idporten-email:user@altinn.xyz";
        ...
        var orderChainRequest = new NotificationOrderChainRequest.NotificationOrderChainRequestBuilder()
            ...
            .SetCreator(new Creator("ttd"))
            .SetType(OrderType.Notification)
            .SetIdempotencyId("self-identified-email-id")
            .SetSendersReference("self-identified-email-ref")
            .SetRequestedSendTime(currentTime.AddMinutes(10))
            .SetRecipient(new NotificationRecipient
            {
                RecipientExternalIdentity = new RecipientExternalIdentity
        ...
        var expectedNotificationOrder = new NotificationOrder
        {
            ...
            Creator = new Creator("ttd"),
            Type = OrderType.Notification,
            SendersReference = "self-identified-email-ref",
            RequestedSendTime = currentTime.AddMinutes(10),
            NotificationChannel = NotificationChannel.Email,
            Recipients = [new Recipient([], externalIdentity: externalIdentity)],
            Templates = [new EmailTemplate("noreply@altinn.no", "Test Subject", "Test Email body", EmailContentType.Plain)]
        };
        ...
        contactPointServiceMock.Verify(
            cp => cp.AddEmailAndSmsContactPointsAsync(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), It.IsAny<OrderLifecycleStage>(), It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);

        contactPointServiceMock.Verify(
            cp => cp.AddPreferredContactPoints(It.IsAny<NotificationChannel>(), It.IsAny<List<Recipient>>(), It.IsAny<string?>(), It.IsAny<OrderLifecycleStage>(), It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUser_ForSmsPreferred_UsesPreferredLookupAndCreatesOrderChain()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        var orderChainRequest = new NotificationOrderChainRequest.NotificationOrderChainRequestBuilder()
            ...
            .SetType(OrderType.Notification)
            .SetRequestedSendTime(currentTime.AddMinutes(10))
            .SetIdempotencyId("self-identified-sms-preferred-id")
        ...
        var expectedSavedOrder = new NotificationOrder
        {
            ...
            SendersReference = "self-identified-sms-preferred-ref",
            Recipients = [new Recipient([], externalIdentity: externalIdentity)],
            Templates =
            [
                new SmsTemplate("Altinn", "SMS body"),
                new EmailTemplate("noreply@altinn.no", "Subject", "Email body", EmailContentType.Plain)
            ]
        };
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(cp => cp.AddPreferredContactPoints(NotificationChannel.SmsPreferred, It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<NotificationChannel, List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((_, recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUser_ForEmailAndSms_UsesEmailAndSmsLookupAndCreatesOrderChain()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        var orderChainRequest = new NotificationOrderChainRequest.NotificationOrderChainRequestBuilder()
            ...
            .SetType(OrderType.Notification)
            .SetRequestedSendTime(currentTime.AddMinutes(10))
            .SetIdempotencyId("self-identified-email-and-sms-id")
        ...
        var expectedSavedOrder = new NotificationOrder
        {
            ...
            SendersReference = "self-identified-email-and-sms-ref",
            Recipients = [new Recipient([], externalIdentity: externalIdentity)],
            Templates =
            [
                new SmsTemplate("Altinn", "SMS body"),
                new EmailTemplate("noreply@altinn.no", "Subject", "Email body", EmailContentType.Html)
            ]
        };
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(cp => cp.AddEmailAndSmsContactPointsAsync(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUserReminderMissingContact_ReturnsProblem()
    {
        // Arrange
        ...
        Guid orderChainId = Guid.NewGuid();
        DateTime currentTime = DateTime.UtcNow;
        const string mainExternalIdentity = "urn:altinn:person:idporten-email:main@altinn.xyz";
        const string reminderExternalIdentity = "urn:altinn:person:idporten-email:reminder@altinn.xyz";
        ...
        contactPointServiceMock
            ...
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
                ...
                foreach (var recipient in recipients)
                {
                    if (recipient.ExternalIdentity == mainExternalIdentity)
                    {
                        recipient.AddressInfo.Add(new EmailAddressPoint("main@altinn.xyz"));
                        recipient.IsReserved = false;
                    }

                    // Intentionally don't add contact for reminder recipient to simulate missing contact
                }
            })
            .Returns(Task.CompletedTask);

        var service = GetTestService(orderRepositoryMock.Object, contactPointServiceMock.Object, mainOrderId, currentTime);
        ...
        contactPointServiceMock.Verify(
            cp => cp.AddSmsContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()),
            Times.Never);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_SelfIdentifiedUserInReminder_CreatesOrderChainWithReminder()
    {
        // Arrange
        Guid reminderId = Guid.NewGuid();
        Guid mainOrderId = Guid.NewGuid();
        ...
        var orderChainRequest = new NotificationOrderChainRequest.NotificationOrderChainRequestBuilder()
            ...
            .SetIdempotencyId("self-identified-reminder-id")
            .SetRequestedSendTime(currentTime.AddMinutes(10))
            .SetRecipient(new NotificationRecipient
            {
                RecipientExternalIdentity = new RecipientExternalIdentity
        ...
        var expectedMainOrder = new NotificationOrder
        {
            ...
            NotificationChannel = NotificationChannel.Email,
            Recipients = [new Recipient([], externalIdentity: externalIdentity)],
            Templates = [new EmailTemplate("noreply@altinn.no", "Main Notification", "Main notification body", EmailContentType.Plain)]
        };

        var expectedReminder = new NotificationOrder
        {
            Id = reminderId,
            Created = currentTime,
            ResourceId = resourceId,
            ...
            NotificationChannel = NotificationChannel.Sms,
            SendingTimePolicy = SendingTimePolicy.Daytime,
            Templates = [new SmsTemplate("Altinn", "Reminder SMS body")],
            Recipients = [new Recipient([], externalIdentity: externalIdentity)]
        };
        ...
        contactPointServiceMock
            .Setup(cp => cp.AddSmsContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
        ...
        // Verify main order used email lookup
        contactPointServiceMock.Verify(
            cp => cp.AddEmailContactPoints(
                It.Is<List<Recipient>>(r => r.Any(rec => rec.ExternalIdentity == externalIdentity)),
                It.Is<string?>(s => s == resourceId),
        ...
        // Verify reminder used SMS lookup
        contactPointServiceMock.Verify(
            cp => cp.AddSmsContactPoints(
                It.Is<List<Recipient>>(r => r.Any(rec => rec.ExternalIdentity == externalIdentity)),
                It.Is<string?>(s => s == resourceId),
    ...
    public async Task RegisterNotificationOrder_ForSmsPreferred_LookupPartialSuccess_OrderCreated()
    {
        ...
        Mock<IContactPointService> contactPointMock = new();
        contactPointMock
            .Setup(cp => cp.AddPreferredContactPoints(input.NotificationChannel, It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<NotificationChannel, List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((_, recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
    ...
    public async Task RegisterNotificationOrder_ForEmailAndSms_CallsAddEmailAndSmsContactPointsAsync()
    {
        ...
        contactPointMock
            .Setup(cp => cp.AddEmailAndSmsContactPointsAsync(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, resourceId, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
                    ...
                    {
                        recipient.AddressInfo.Add(new SmsAddressPoint("+4799999999"));
                        recipient.AddressInfo.Add(new EmailAddressPoint("first-recipient@altinn.xyz"));
                    }
                    else if (recipient.NationalIdentityNumber == "30286043298")
                    {
                        recipient.AddressInfo.Add(new SmsAddressPoint("+4788888888"));
                        recipient.AddressInfo.Add(new EmailAddressPoint("second-recipient@altinn.xyz"));
                    }
                }
            })
            .Returns(Task.CompletedTask);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_ShouldReturnProblem_WhenMissingContactInformation()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(contactService => contactService.AddEmailContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_ShouldReturnProblem_WhenMissingContactInformationForReminder()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
    ...
    [InlineData("   ", "")]
    [InlineData("tax-2025", "   ")]
    [InlineData("urn:altinn:resource:tax-2025", "read")]
    [InlineData("urn:altinn:resource:tax-2025", "access")]
    [InlineData("urn:altinn:resource:tax-2025", "write")]
    public async Task RegisterNotificationOrderChain_RecipientPersonWithMultipleReminders_OrderChainCreated(string resourceId, string? resourceAction)
    {
        // Arrange
        Guid mainOrderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        contactPointServiceMock
            .Setup(contactService => contactService.AddPreferredContactPoints(It.IsAny<NotificationChannel>(), It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<NotificationChannel, List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((channel, recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
    ...
    public async Task RegisterNotificationOrderChain_RecipientOrganizationWithEmailAndSms_SendingTimePolicyRespectsSmsOverEmail()
    {
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(contactService => contactService.AddEmailAndSmsContactPointsAsync(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
                foreach (var recipient in recipients)
                {
        ...
        Assert.Equal("REF-42DBDAB8281C", result.Value.OrderChainReceipt.SendersReference);
    ...
    [Fact]
    public async Task RegisterNotificationOrderChain_WithMissingSmsContactInformation_ReturnsProblem()
    {
        // Arrange
        Guid orderId = Guid.NewGuid();
        Guid orderChainId = Guid.NewGuid();
        ...
        var contactPointServiceMock = new Mock<IContactPointService>();
        contactPointServiceMock
            .Setup(contactService => contactService.AddSmsContactPoints(It.IsAny<List<Recipient>>(), It.IsAny<string?>(), OrderLifecycleStage.Registration, It.IsAny<Creator>(), It.IsAny<string?>()))
            .Callback<List<Recipient>, string?, OrderLifecycleStage, Creator, string?>((recipients, _, _, _, _) =>
            {
    ...
    public async Task RetrieveOrderChainTracking_WhenOrderChainExists_ReturnsResponse()
    {
        ...
        // Act
        var result = await service.RetrieveOrderChainTracking(creatorName, idempotencyId, TestContext.Current.CancellationToken);
