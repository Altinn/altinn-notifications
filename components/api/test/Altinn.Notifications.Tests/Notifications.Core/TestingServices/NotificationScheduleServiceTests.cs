...
namespace Altinn.Notifications.Tests.Notifications.Core.TestingServices
{
    public class NotificationScheduleServiceTests
    {
        ...
        [Fact]
        public void GetRequestedSendTimeForDaytimeSendCondition_WhenRequestedTimeIsWithinSendWindow_ReturnsUnchanged()
        {
            // Arrange
            var requestedSendTime = new DateTime(2025, 08, 25, 10, 0, 0, DateTimeKind.Utc); // within 09-17 local window

            // Act
            var result = _notificationScheduleService.GetRequestedSendTimeForDaytimeSendCondition(requestedSendTime);

            // Assert
            Assert.Equal(requestedSendTime, result);
        }

        [Fact]
        public void GetRequestedSendTimeForDaytimeSendCondition_WhenRequestedTimeIsBeforeSendWindow_ReturnsUnchanged()
        {
            // Arrange
        ...
        [Fact]
        public void GetRequestedSendTimeForDaytimeSendCondition_WhenRequestedTimeIsAfterSendWindow_ReturnsNextDayWindowStart()
        {
            // Arrange
            var requestedSendTime = new DateTime(2025, 08, 25, 20, 0, 0, DateTimeKind.Utc); // 20:00 UTC is after window closes
            ...
            // Assert
            Assert.Equal(expectedSendTime, result);
        }
    }
}
