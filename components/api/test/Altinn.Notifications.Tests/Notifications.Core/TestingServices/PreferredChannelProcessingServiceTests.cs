...
public class PreferredChannelProcessingServiceTests
{
    ...
    /// </summary>
    [Fact]
    public async Task ProcessOrderRetry_EmailPreferred_CallsRetryMethodsAndReturnsCorrectResult()
    {
        // Arrange
        ...
        _contactPointMock
            .Setup(cp => cp.AddPreferredContactPoints(order.NotificationChannel, It.IsAny<List<Recipient>>(), It.IsAny<string?>(), It.IsAny<OrderLifecycleStage>(), It.IsAny<bool>(), It.IsAny<string?>()))
        ...
        // Act
        var result = await service.ProcessOrderRetry(order);
    ...
    public async Task ProcessOrderRetry_SmsPreferred_CallsRetryMethods()
    {
        ...
        _emailProcessingMock
            ...
            .Returns(Task.FromResult(expectedEmailOrderProcessingResult));

        _smsProcessingMock
            .Setup(x => x.ProcessOrderRetryWithoutAddressLookup(
                It.IsAny<NotificationOrder>(),
                It.Is<List<Recipient>>(l => l.Count == 3 && !l.Exists(r => r.NationalIdentityNumber == "22012047278"))))
            .Returns(Task.FromResult(expectedSmsOrderProcessingResult));
        ...
        // Assert
        Assert.Same(expectedEmailOrderProcessingResult, result.EmailOrderProcessingResult);
        Assert.Same(expectedSmsOrderProcessingResult, result.SmsOrderProcessingResult);
        _smsProcessingMock.Verify(x => x.ProcessOrderRetryWithoutAddressLookup(It.IsAny<NotificationOrder>(), It.IsAny<List<Recipient>>()), Times.Once);
        _emailProcessingMock.Verify(x => x.ProcessOrderRetryWithoutAddressLookup(It.IsAny<NotificationOrder>(), It.IsAny<List<Recipient>>()), Times.Once);
    }
    ...
    public async Task ProcessOrder_WithSupportedChannel_DoesNotThrowArgumentOutOfRangeException(NotificationChannel channel)
    {
        // Arrange
        var order = new NotificationOrder
        {
