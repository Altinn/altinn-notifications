...
public class EmailAndSmsOrderProcessingService : IEmailAndSmsOrderProcessingService
{
    ...
    private async Task ProcessOrderInternal(NotificationOrder order, bool isRetry)
    {
        ...
        {
            await _contactPointService.AddEmailAndSmsContactPointsAsync(recipientsWithoutContactPoint, order.ResourceId, OrderLifecycleStage.Processing, order.Creator, order.ResourceAction);
        }
