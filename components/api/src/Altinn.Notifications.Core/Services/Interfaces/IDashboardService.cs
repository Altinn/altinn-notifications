...
public interface IDashboardService
{
    ...
    /// <summary>
    /// Retrieves all email notifications for a recipient identified by their email address within a given date range.
    /// If no date range is provided, defaults to the last 7 days.
    /// </summary>
    /// <param name="email">The email address of the recipient. Matched case-insensitively.</param>
    /// <param name="dateTimeFrom">Start of the date range (inclusive). Defaults to 7 days ago if null.</param>
    /// <param name="dateTimeTo">End of the date range (exclusive). Defaults to now if null.</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A list of <see cref="DashboardNotification"/> matching the search criteria, or a <see cref="ServiceError"/>.</returns>
    Task<Result<List<DashboardNotification>, ServiceError>> GetNotificationsByEmailAsync(string email, DateTime? dateTimeFrom, DateTime? dateTimeTo, CancellationToken cancellationToken);

    /// <summary>
    /// Retrieves all SMS notifications for a recipient identified by their phone number within a given date range.
    /// If no date range is provided, defaults to the last 7 days.
    /// </summary>
    ...
    /// <summary>
    /// Retrieves all notifications (email and SMS) belonging to a shipment identified by its shipment id within a given date range.
    /// If no date range is provided, defaults to the last 7 days.
    /// </summary>
    ...
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A list of <see cref="DashboardNotification"/> matching the search criteria, or a <see cref="ServiceError"/>.</returns>
    Task<Result<List<DashboardNotification>, ServiceError>> GetNotificationsByShipmentIdAsync(string shipmentId, DateTime? dateTimeFrom, DateTime? dateTimeTo, CancellationToken cancellationToken);
}
