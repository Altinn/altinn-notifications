...
public class PreferredChannelProcessingService : IPreferredChannelProcessingService
{
    ...
    private async Task ProcessOrderInternal(NotificationOrder order, bool isRetry)
    {
        ...
        {
            await _contactPointService.AddPreferredContactPoints(order.NotificationChannel, recipientsWithoutContactPoint, order.ResourceId, OrderLifecycleStage.Processing, order.Creator, order.ResourceAction);
        }
